...
class CurrencyHelper {
  static String formatCompactAmount(dynamic numValue) {
    ...
    // 1. Nếu từ 1 tỷ trở lên -> Rút gọn chữ B
    if (value >= 1000000000) {
      ...
      return '${formatted}B'; // ✨ Thêm ₫ cho đồng bộ
    }

    // 2. Nếu từ 1 triệu trở lên -> Rút gọn chữ M
    if (value >= 1000000) {
      double shortValue = value / 1000000;
      String formatted = shortValue % 1 == 0
          ? shortValue.toStringAsFixed(0)
          : shortValue.toStringAsFixed(2);
      return '${formatted}M'; // ✨ Thêm ₫ cho đồng bộ
    }

    // 3. Nếu dưới 1 triệu -> Giữ nguyên số 0 phân tách dấu chấm, viết liền '₫'
    final formatter = NumberFormat('#,###', 'vi_VN');
    return '${formatter.format(value.toInt())}'; // 🔴 Sửa lỗi: Đã thêm '₫' vào đây nè bồ tèo!
  }
}
